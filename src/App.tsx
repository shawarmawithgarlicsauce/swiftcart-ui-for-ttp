...
function AppContent() {
  ...
  const [showQuickCartView, setShowQuickCartView] = useState(false);
  ...
  useEffect(() => {
    if (currentScreen === 'home') {
      const timer = setTimeout(() => {
        ...
        if (Math.random() > 0.7 && scannedItems.length < 5) { // 30% chance
          if (existingItem) {
  ...
  const scannedItemsCount = scannedItems.reduce((sum, item) => sum + item.quantity, 0);
    ...
    <>
      ...
      {currentScreen === 'home' && (
        <HomeScreen
          ...
          onNavigateToScannedCart={() => setCurrentScreen('scannedCart')}
      ...
      {currentScreen === 'comparison' && (
        <BrandComparison
          items={mockItems.filter(item => item.category === comparisonCategory)}
      ...
      {currentScreen === 'home' && scannedItemsCount > 0 && (
        <div className="fixed bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-white via-white to-transparent pointer-events-none z-40">
          <div className="max-w-4xl mx-auto pointer-events-auto">
            ...
            >
              <span className="text-xl">View Cart & Checkout</span>
        ...
        <div className="fixed bottom-6 left-6 bg-white rounded-lg shadow-xl p-4 max-w-xs z-40">
          ...
          <div className="space-y-1 text-sm">
            ...
            {scannedItems.length > 3 && (
              <p className="text-gray-500 text-xs">+{scannedItems.length - 3} more items</p>
            )}
          </div>
